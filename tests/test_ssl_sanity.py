...
def test_simclr_overfit_small_batch():
    ...
    x1 = torch.randn(n_samples, 1, n_mels, n_frames)
    x2 = x1 + 0.01 * torch.randn(n_samples, 1, n_mels, n_frames)
...
def test_simclr_accuracy_improves():
    ...
    base = torch.randn(n_samples, 1, n_mels, n_frames)
    
    # Create two augmented views with slight noise
    x1 = base + 0.1 * torch.randn(n_samples, 1, n_mels, n_frames)
    x2 = base + 0.1 * torch.randn(n_samples, 1, n_mels, n_frames)
