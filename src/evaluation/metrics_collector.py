...
class MetricsCollector:
    ...
    def add_batch(
        self, preds: np.ndarray, labels: np.ndarray, probs: np.ndarray = None
    ):
    ...
    def compute_metrics(self) -> Dict[str, Any]:
        ...
        # Confusion matrix
        cm = confusion_matrix(
            all_labels, all_preds, labels=range(len(self.class_names))
        )
        ...
        self.metrics = {
            ...
            "per_class_precision": {
                self.class_names[i]: float(precision[i])
                for i in range(len(self.class_names))
            },
            "per_class_recall": {
                self.class_names[i]: float(recall[i])
                for i in range(len(self.class_names))
            },
            "per_class_f1": {
                self.class_names[i]: float(f1[i]) for i in range(len(self.class_names))
            },
            "per_class_support": {
    ...
    def plot_confusion_matrix(
        self, figsize: Tuple[int, int] = (12, 10), filepath: Path = None
    ):
    ...
    def generate_markdown_report(self, filepath: Path = None):
        ...
        report = (
            ...
            f"| Weighted F1 | {self.metrics['weighted_f1']:.4f} |\n"
            f"\n"
            f"## Per-Class Metrics\n"
            f"\n"
        ...
        report += (
            ...
            f"\n"
            f"## Per-Class Metrics Visualization\n"
            f"\n"
            f"![Per-Class Metrics](per_class_metrics.png)\n"
        )
