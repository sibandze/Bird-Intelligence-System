...
class SimCLRExperimentTrainer:
    ...
    def __init__(self, config: Dict[str, Any], run_dir: Path,
        ...
        if callbacks is None:
            callbacks = [
                ...
                EarlyStoppingCallback(monitor="val_loss", mode="min",
    ...
    def get_dataloaders(self, df):
        ...
        train_loader = DataLoader(
            ...
            persistent_workers=num_workers > 0, collate_fn=simclr_collate_fn
        )
    ...
    def train(self, df):
        ...
        scheduler = create_scheduler(
            optimizer=optimizer, scheduler_type=scheduler_type,
        ...
        for epoch in range(resume_epoch, epochs):
            ...
            val_acc_total = 0.0
            with torch.no_grad():
                for x1, x2 in tqdm(val_loader, desc=f"Epoch {epoch+1}/{epochs} [Val]", leave=False):
                    x1, x2 = x1.to(self.device), x2.to(self.device)
                    with self.precision.autocast():
